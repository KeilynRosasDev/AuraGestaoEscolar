// src/components/SecretariaHeader.tsx
import React, { useState, useEffect } from 'react';
import { Menu, X, ChevronRight, Home, Settings, Info, LogIn } from 'lucide-react';
import { Link, useLocation, useNavigate } from 'react-router-dom';

const SecretariaHeader: React.FC = () => {
  ...
  const menuItems = [
    ...
    },
    {
      ...
      description: 'Área da secretaria'
    }
  ];
        ...
        <nav className="container-custom">
          <div className="flex items-center justify-between h-20 px-4">
            ...
            <div className="hidden md:flex items-center space-x-3">
              ...
              {/* Botão Entrar - Estilo mais limpo e profissional */}
              <button
                onClick={handleLogin}
                className="relative px-4 py-2 rounded-lg font-medium transition-all duration-200 flex items-center space-x-2 bg-gradient-to-r from-blue-600 to-blue-700 text-white hover:shadow-md hover:scale-[1.02] shadow-sm min-w-[100px] justify-center group"
              >
                <LogIn size={16} className="text-white group-hover:animate-pulse" />
                <span className="font-medium">Entrar</span>
                
                {/* Efeito de brilho no hover */}
                <div className="absolute inset-0 rounded-lg opacity-0 group-hover:opacity-100 transition-opacity duration-300 bg-gradient-to-r from-white/20 to-transparent"></div>
              </button>
            </div>
            ...
            <div className="md:hidden bg-white border-t border-gray-100 shadow-xl">
              <div className="px-4 py-4 space-y-2">
                ...
                <div className="pt-4 border-t border-gray-100">
                  <button
                    onClick={handleLogin}
                    className="w-full flex items-center justify-center space-x-2 px-3 py-3 rounded-lg font-medium transition-all duration-300 text-sm bg-gradient-to-r from-blue-600 to-blue-700 text-white hover:shadow-md"
                  >
                    <LogIn size={16} />
                    <span>Acessar Área da Secretaria</span>
                  </button>
                </div>
              </div>
            </div>
